...
export class NextGenRuntime {
  ...
  private storyChapter = 'Awakening in the Overworld';
  private civilians = 0;
  private pirates = 0;
  private rockets = 0;
  ...
  update(deltaSeconds: number, playerPosition: Vector3, settings: GameSettings): void {
    ...
    this.sun.position.x = playerPosition.x + Math.cos(now * 0.00008) * 42;
    this.sun.position.z = playerPosition.z + Math.sin(now * 0.00008) * 42;
    this.sun.position.y = playerPosition.y + 42 + Math.sin(now * 0.00006) * 8;
    ...
    if (this.fireworksUntil > now && settings.particlesEnabled) {
      // Keep temporary particle systems alive; they auto-expire by lifetime.
    }
  }
  ...
  getStatus(): NextGenStatus {
    return {
      ...
      blackHoles: 1,
      cityBiome: 'Auralis Megacity Biome',
      cityLengthKm: 400,
  ...
  private createSolarSystem(spawn: SpawnPoint): void {
    ...
    names.forEach((name, i) => {
      ...
      planet.material = mat;
      planet.position = new Vector3(spawn.x + 15 + i * 2.2, spawn.y + 18 + i * 0.8, spawn.z + 18);
      planet.isPickable = false;
      this.runtimeMeshes.push({ mesh: planet, orbitCenter: new Vector3(spawn.x, spawn.y + 20, spawn.z), orbitRadius: 15 + i * 2.4, orbitSpeed: 0.00005 + i * 0.00001 });
    });
    ...
    blackHole.position = new Vector3(spawn.x - 24, spawn.y + 18, spawn.z + 24);
    blackHole.material = blackHoleMat;
    blackHole.isPickable = false;
    this.runtimeMeshes.push({ mesh: blackHole, bob: 9 });
  }

  private createMegaCityBiome(spawn: SpawnPoint): void {
    ...
    const baseX = spawn.x + 34;
    const baseZ = spawn.z + 26;
  ...
  private createWaterPiratesAndInfrastructure(spawn: SpawnPoint): void {
    ...
    for (let i = 0; i < 4; i += 1) {
      ...
      boat.position = new Vector3(spawn.x - 18 + i * 3, spawn.y - 1.1, spawn.z + 8 + (i % 2) * 2);
    ...
    dam.position = new Vector3(spawn.x - 23, spawn.y + 0.4, spawn.z + 3);
    ...
    plant.position = new Vector3(spawn.x + 27, spawn.y + 1.6, spawn.z + 8);
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      sewer.position = new Vector3(spawn.x + 18 + i * 2, spawn.y - 0.8, spawn.z + 18);
  ...
  private createEnderAbyssStorySet(spawn: SpawnPoint): void {
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      island.position = new Vector3(spawn.x - 34 + i * 5, spawn.y + 9 + (i % 2) * 2, spawn.z - 34 + i * 2);
    ...
    dragon.position = new Vector3(spawn.x - 23, spawn.y + 17, spawn.z - 30);
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      tentacle.position = new Vector3(spawn.x - 18 + i * 1.2, spawn.y + 8, spawn.z - 38);
  ...
  private createPhysicsVisuals(spawn: SpawnPoint): void {
    ...
    cloth.position = new Vector3(spawn.x + 11, spawn.y + 3.2, spawn.z - 12);
  ...
  private createMarketplaceShowcase(spawn: SpawnPoint): void {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      kiosk.position = new Vector3(spawn.x + 3 + i * 3, spawn.y + 1, spawn.z + 15);
  ...
  private createRareMcdonaldsWorld(spawn: SpawnPoint): void {
    ...
    restaurant.position = new Vector3(spawn.x + 18, spawn.y + 1.5, spawn.z - 18);
    ...
    for (let i = 0; i < 2; i += 1) {
      ...
      arch.position = new Vector3(spawn.x + 16.8 + i * 2.4, spawn.y + 4.2, spawn.z - 20.6);
    ...
    foods.forEach((food, i) => {
      ...
      item.position = new Vector3(spawn.x + 16 + i * 1.5, spawn.y + 1.1, spawn.z - 14.5);
  ...
  private createSmoke(spawn: SpawnPoint): void {
    const ps = new ParticleSystem('nextgen_fire_smoke_particles', 500, this.scene);
    ps.particleTexture = new Texture('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=', this.scene);
    ps.emitter = new Vector3(spawn.x + 20, spawn.y + 1.5, spawn.z + 8);
    ps.minLifeTime = 1.4;
    ps.maxLifeTime = 4.4;
    ps.emitRate = 70;
    ps.minSize = 0.15;
