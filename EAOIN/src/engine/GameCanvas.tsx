  ...
  useEffect(() => {
    ...
    const initializeWorld = async (): Promise<void> => {
      ...
      const handleKeyDown = (event: KeyboardEvent): void => {
        ...
        if (event.key.toLowerCase() === 'r') { event.preventDefault(); const m = worldInteractions.tryLaunchRocket(camera.position, () => { dimensionRuntime.setDimension('moon'); dimensionRuntime.triggerTransitionEffect(camera.position, true); nextGenRuntime.launchMoonRuntime(); }); audio.play('ui', settingsRef.current); showActionMessage(m); if (m.includes('launched')) { const sling = endBlackHole.gravitySlingshot(camera.position); if (sling) showActionMessage(`🪐 Gravity slingshot! The black hole whips your rocket to ${sling} launch power.`); } publishRuntimeStatus(); return; }
