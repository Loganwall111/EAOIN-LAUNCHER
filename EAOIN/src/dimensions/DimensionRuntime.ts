/** Runtime dimension controller: visible portals + scene rule changes. De-cluttered to 70+ blocks. */
...
export class DimensionRuntime {
  ...
  constructor(private readonly scene: Scene, spawn: SpawnPoint, seed: string) {
  ...
  applyCurrent(): RuntimeDimensionDefinition {
    ...
    // Improved: set scene gravity for reference, but actual player gravity handled in GameCanvas with custom velocity integrating this value
  ...
  triggerTransitionEffect(position: Vector3, usedPlacedCore: boolean): void {
    ...
    const ring = MeshBuilder.CreateTorus('dimension_transition_ring', { diameter: usedPlacedCore ? 3.3 : 2.4, thickness: 0.09, tessellation: 40 }, this.scene);
    ring.position = position.add(new Vector3(0, -0.6, 0));
    ring.rotation.x = Math.PI / 2;
    ring.material = material;
    ring.isPickable = false;
    this.transitionMeshes.push({ mesh: ring, ttl: 1.5 });
  }
  ...
  private createDimensionOverlays(spawn: SpawnPoint): void {
    const pc = this.layout.portalCore;
    ...
    for (let i = 0; i < 8; i += 1) {
      ...
      spike.position = new Vector3(pc.x + Math.cos(angle) * 7.5, spawn.y - 1.0, pc.z + Math.sin(angle) * 7.5);
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      obelisk.position = new Vector3(pc.x + Math.cos(angle) * 9, spawn.y + 0.1, pc.z + Math.sin(angle) * 9);
    ...
    for (let i = 0; i < 9; i += 1) {
      ...
      crater.position = new Vector3(pc.x + Math.cos(angle) * 11, spawn.y - 1.65, pc.z + Math.sin(angle) * 11);
  ...
  private createPortalMonument(spawn: SpawnPoint): void {
    ...
    const base = MeshBuilder.CreateTorus('dimension_portal_ring', { diameter: 2.8, thickness: 0.16, tessellation: 36 }, this.scene);
    base.position = new Vector3(pc.x, spawn.y + 0.8, pc.z);
    base.rotation.x = Math.PI / 2;
    base.material = material;
    base.isPickable = false;

    const core = MeshBuilder.CreateCylinder('dimension_portal_core', { height: 0.12, diameter: 1.65, tessellation: 36 }, this.scene);
    core.position = base.position.clone();
    core.material = material;
    core.isPickable = false;

    // beacon
    const beacon = MeshBuilder.CreateBox('portal_beacon', { width: 0.2, height: 3.2, depth: 0.2 }, this.scene);
    beacon.position = new Vector3(pc.x, spawn.y + 2.5, pc.z);
    const bMat = new StandardMaterial('portal_beacon_mat', this.scene);
    bMat.emissiveColor = new Color3(0.2, 0.6, 1);
    bMat.alpha = 0.5;
    beacon.material = bMat;
    beacon.isPickable = false;

    this.portalMeshes.push(base, core, beacon);
  }
}
